...
// Import the methods from the MySQL and MongoDB files
const {
  getStudents,
  addStudent,
  getStudentById,
...
// Import the methods from the MongoDB file
const {
  getLecturers,
  deleteLecturerById,
  getLecturerById,
...
//app.get method to get the students
...
app.get("/students/update/:id", async (req, res) => {
  try {
    const student = await getStudentById(req.params.id); // Call the MySQL method
    res.render("updatestudent", { student }); // Render the EJS page with data
  } catch (err) {
    console.error("Error retrieving student:", err);
    res.status(500).send("Error retrieving student from database");
  }
...
app.post("/students/update", async (req, res) => {
  const { sid, name, age } = req.body; // Get the form data
  try {
    await updateStudent(sid, name, age); // Call the MySQL method
...
//method to get the lecturer by id
app.get("/lecturers/update/:id", async (req, res) => {
  try {
    const lecturer = await getLecturerById(req.params.id); // Call the MongoDB method
    res.render("updatelecturer", { lecturer }); // Render the EJS page with data
  } catch (err) {
...
//method to update the lecturer by id
app.post("/lecturers/update/:id", async (req, res) => {
  const { id, name, did } = req.body; // Get the form data
  try {
...
//method to render the addlecturer page
